...
手机上看到来不及读的公众号文章，复制链接、点一下桌面小组件，约一分钟后 Notion 里就出现一条带 **AI 摘要、关键词、洞见、阅读优先级**的记录，页面正文是全文快照；同时在本仓库留一份 Markdown 归档。
...
全程约 20 分钟。四个部分按顺序做，中间有几个坑标了 ⚠️，跳过会白折腾。
...
- `AI_BASE_URL` 填 `https://x.com` 或 `https://x.com/v1` 都可以，代码会自动拼对
...
**抓取依赖页面结构**：微信改版可能导致解析失效。解析逻辑集中在 `src/clipper/fetcher.py`，有 HTML fixture 测试兜底，改一处即可。
