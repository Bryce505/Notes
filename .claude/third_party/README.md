...
这些内容是从对应上游仓库的 `skills/`(和 `hooks/`)子目录直接搬运/改写而来,
**没有**通过 Claude Code 的 plugin/marketplace 安装机制安装(那条路径在本仓库的沙箱
会话里因为 marketplace 从未被拉取而未生效,详见对话记录),而是作为项目级 skill/hook
直接提交进本仓库,任何 clone 本仓库的 VM/会话都会自动识别到。
...
- 上游仓库:https://github.com/KKKKhazix/khazix-skills(取其中 `neat-freak` 子目录)
- 拉取时的 commit:`7a5c4934be4106ac740ffdb95280bb81b3f4b83c`
- 许可证:MIT(见 `neat-freak/LICENSE`,版权归"数字生命卡兹克")
- 搬运内容:`neat-freak/{SKILL.md,references/,scripts/}` 原样复制到
  `.claude/skills/neat-freak/`。上游目录里的 `evals/`(约 632K 的评测用
  fixture,供开发者跑技能评测用)**未搬运**——运行时不需要,只是用来验证
  技能质量的测试数据。纯 skill,没有 hooks/commands 需要额外接线。

## 维护提示

- 这两份内容是"一次性搬运"的快照,不会随上游更新自动同步。以后要升级,重新克隆
